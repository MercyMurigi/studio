...
export default function LawyerWalletPage() {
  ...
  useEffect(() => {
    // Simulate fetching current lawyer profile and their transactions
    const lawyer = mockLawyerProfiles[0]; // Assuming the first lawyer is the logged-in one
    setCurrentLawyer(lawyer);
    // In a real app, transactions would be filtered by lawyer.id
    setTransactions(mockLawyerWalletTransactions.filter(tx => tx.amountHaki !== 0)); // Display non-zero transactions
  }, []);
  ...
  const balanceHaki = currentLawyer.walletBalanceHaki || 0;
  const balanceUsdEquivalent = balanceHaki / HAKI_CONVERSION_RATE;

  return (
    <>
      <PageTitle
        title={`${currentLawyer.name}'s Wallet`}
